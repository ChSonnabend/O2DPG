...
### OWN STUFF
parser.add_argument('--run-with-ideal-clusterizer', default=1, help='Run TPC with ideal clusterizer at sim-time')
parser.add_argument('--mc-clusterization-mode', default=2, help="Mode of the ideal clusterizer (0=disabled, 1=ideal clusterizer, 2=ideal clusterizer with full readout)")
...
for tf in range(1, NTIMEFRAMES + 1):
   ...
   TPCDigitask['cmd'] += '${O2_ROOT}/bin/o2-sim-digitizer-workflow ' + getDPL_global_options(bigshm=True) + ' -n ' + str(args.ns) + simsoption       \
                         ...
                         + putConfigValues(["TPCGasParam","TPCGEMParam","TPCEleParam","TPCITCorr","TPCDetParam"], localCF=tpcLocalCF)
   if args.run_with_ideal_clusterizer:
      TPCDigitask['cmd'] += ' --mc-clusterization-mode ' + str(args.mc_clusterization_mode) \
                         + ' --ideal-clusterizer-padsize ' + str(args.ideal_clusterizer_padsize) \
                         + ' --ideal-clusterizer-timesize ' + str(args.ideal_clusterizer_timesize)
